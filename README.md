...
- `code/preprocessing/`: preprocessing scripts for PoC report text.
- `code/RGFP/`: implementation of RGFP-based field extraction.
- `code/assessment/consistency/`: consistency assessment of Type, Platform, and CVE-ID.
- `code/assessment/completeness/`: completeness assessment of standardized headers and exploit code.
- `code/assessment/accuracy/`: cross-source accuracy assessment of Type and Platform.
- `code/assessment/currentness/`: publication-delay and currentness analysis.
- `code/MSVI/`: implementation of multi-source verification, field correction, and header completion.
- `prompts/`: prompts used for LLM-based field extraction and completion.
- `data/`: dataset descriptions, sample data, and preprocessing information.
- `results/`: intermediate and final experimental outputs.
...
The prompts used by RGFP are provided in:
...
This repository is intended for academic research on vulnerability-report data quality.

Please note:

- Do not upload API keys or database credentials.
- Do not upload private MongoDB configuration files.
- Do not expose local file paths containing personal information.
- Raw PoC data should be handled carefully because some reports may contain executable exploit code.
- Follow the terms of use and redistribution policies of the original data sources.
